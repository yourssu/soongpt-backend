...
def test_settings_default_values():
    """기본 설정값 테스트"""
    # 개발 모드에서는 internal_jwt_secret이 None이어도 됨
...
def test_settings_production_requires_jwt_secret():
    ...
    with pytest.raises(ValueError) as exc_info:
        Settings(debug=False, internal_jwt_secret=None)
...
def test_settings_is_production_property():
    ...
    dev_settings = Settings(debug=True, internal_jwt_secret="test")
    prod_settings = Settings(debug=False, internal_jwt_secret="test")
