...
def create_custom_target_map() -> Dict[int, str]:
    ...
    This replaces the need for a separate generate_custom_target_map.py script.
...
def generate_target_inserts(target_map: Dict, course_data: List[Dict], custom_target_map: Dict[int, str]) -> List[str]:
    """
    Generate SQL INSERT statements for TargetEntity from target_map and custom mappings.
    
    This function:
    1. Takes the original target field from course_data (from course_parser.py output)
    2. Checks for custom target mappings first (by course code)
    3. Falls back to target_map.json for standard mappings
    4. Generates SQL INSERT statements for the target table
    ...
    Args:
        ...
        custom_target_map: Custom target mappings by course code
    ...
    insert_statements.append("-- Generated from target_map.json mapping with custom overrides")
    ...
    for course in course_data:
        ...
        # Check for custom target mapping first (by course code)
        mapped_target_value = None
        if code in custom_target_map:
            mapped_target_value = custom_target_map[code]
            insert_statements.append(f"-- Custom target for course {code}: {mapped_target_value}")
        else:
...
def main():
    ...
    custom_target_map = create_custom_target_map()
    
    print("Loading course data...")
    course_data = load_course_data()
    
    print("Generating SQL INSERT statements...")
    insert_statements = generate_target_inserts(target_map, course_data, custom_target_map)
