...
# Usage:
#   SERVER_NAME=api.soongpt.yourssu.com SERVER_PORT=9000 ./ops/nginx/install-prod.sh

SERVER_NAME="${SERVER_NAME:-api.soongpt.yourssu.com}"
SERVER_PORT="${SERVER_PORT:-9000}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TEMPLATE_PATH="$SCRIPT_DIR/soongpt.prod.conf.template"
...
# We only substitute SERVER_NAME and SERVER_PORT.
# Note: The deployment script will later use sed to change the port in proxy_pass.
